...
def discrete_grads(loss,network,LR):
    ...
    for param, parambest in izip(W_params, best_params) :
        ...
        delta_W1 =c* (state_rand-parambest) #parambest would transfer to state_rand with probability of 1-a, or keep unmoved with probability of a
	...
	Prob1 = T.tanh(th*Prob1) #the nonlinear tanh() function accelerates the state transfer
        ...
        Gate2 = T.cast(srng.binomial(n=1, p=Prob2, size=T.shape(Prob2)), theano.config.floatX) # Gate1 is a binary variable with probability of Prob2 to be 1
...
def train(  network,
	...
	# otherwise, the network will be normally trained
        ...
        #print("  best_update:                   "+str(best_update)) 
...
"""
all the attributes in one layer class cannot be set more than once.
"""
